...
# ── Hyperparameters ──────────────────────────────────────────────────────────
STAGE1_EPOCHS = 25   # 🔽 reduced (avoid overfitting)
STAGE2_EPOCHS = 40
BATCH_SIZE    = 32
STAGE1_LR     = 1e-3
STAGE2_LR     = 1e-5
PATIENCE      = 7    # 🔽 faster stopping
...
def get_callbacks(stage: int, model_name: str):
    ...
    callbacks = [
        ...
        tf.keras.callbacks.TensorBoard(
            ...
            histogram_freq=0   # 🔽 disable heavy logging (faster)
        ),
...
def train():
    ...
    log.info(f"Stage 1 complete → {ckpt_s1}")
    ...
    log.info(f"Stage 2 complete → {ckpt_s2}")

    # ── Evaluation ───────────────────────────────────────────────────────────
    evaluate_model(model, test_ds, label="Final Model")
